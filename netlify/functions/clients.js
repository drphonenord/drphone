  ...
  try {
    ...
    if (event.httpMethod === 'GET') {
      ...
      if (itemId) {
        ...
        return item ? ok(item) : error('clients: not found', 404);
      }
      return ok({ items });
    }
    ...
    if (event.httpMethod === 'PUT') {
      ...
      if (idx === -1) return error('clients: not found', 404);
    ...
    if (event.httpMethod === 'DELETE') {
      ...
      if (filtered.length === items.length) return error('clients: not found', 404);
    ...
    console.error('Function clients error:', e);
